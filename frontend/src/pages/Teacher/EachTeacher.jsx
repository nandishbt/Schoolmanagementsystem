...
import { deleteTeacher, getOneTeacher, updateTeacher, updateTeacherProfile } from '../../utils/api'
...
const navigate = useNavigate()
  ...
  return (
    ...
    <dl className="-my-3 divide-y divide-gray-100 text-sm">
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        ...
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.name}</dd>
      </div>
  
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Subject</dt>
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.assignedSubject}</dd>
      </div>
  
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Gender</dt>
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.gender}</dd>
      </div>
  
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Date Of Birth</dt>
        <dd className="text-gray-700 sm:col-span-2">{new Date(data?.teacher.dob).toLocaleDateString('en-IN')}</dd>
      </div>

      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Phone</dt>
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.phone}</dd>
      </div>
      ...
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        ...
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.email}</dd>
      </div>

      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Salary</dt>
        <dd className="text-gray-700 sm:col-span-2">{data?.teacher.salary}</dd>
      </div>
      ...
      <div className="grid grid-cols-1 gap-1 py-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-4">
        <dt className="font-medium text-gray-900">Assigned Classes</dt>
        {
          data?.gradesTeachertaking.map((teacher, index)=>(
            <dd key={index} className="text-gray-700 sm:col-span-2">{teacher.grade}</dd>
          ))
        }
  ...
  <div onClick={deleteThisTeacher}  className="flex justify-center items-center bottom-5 fixed">
        <DeleteBtn />
      </div>
  </div>
  )
